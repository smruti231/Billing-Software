...
class BillClass:
    def __init__(self,root):
       ...
       # ===title=====
        ...
        self.Product_Table = ttk.Treeview(ProductFrame3, columns=("pid", "name", "price", "qty","status"),yscrollcommand=scrolly.set, xscrollcommand=scrollx.set)  # tupple banaye hai
        ...
        Cal_Frame=Frame(Cal_Cart_Frame,bd=9,relief=RIDGE,bg="ivory3")
        Cal_Frame.place(x=5,y=10,width=268,height=440)

        txt_cal_input=Entry(Cal_Frame,textvariable=self.var_cal_input,font=('arial',15,'bold'),width=21,bd=10,relief=GROOVE,stat='readonly',justify=RIGHT)
        txt_cal_input.grid(row=0,columnspan=4)

        btn_7=Button(Cal_Frame,text='7',command=lambda:self.get_input(7),font=('arial',12,'bold'),bd=5,width=4,pady=10,cursor="hand2").grid(row=1,column=0)
        btn_8 = Button(Cal_Frame, text='8', command=lambda:self.get_input(8),font=('arial', 12, 'bold'), bd=5, width=4, pady=10, cursor="hand2").grid( row=1, column=1)
        btn_9= Button(Cal_Frame, text='9',command=lambda:self.get_input(9), font=('arial', 12, 'bold'), bd=5, width=4, pady=10, cursor="hand2").grid(row=1, column=2)
        btn_sum= Button(Cal_Frame, text='+',command=lambda:self.get_input('+'), font=('arial', 12, 'bold'), bd=5, width=4, pady=10, cursor="hand2").grid(row=1, column=3)



        btn_4=Button(Cal_Frame,text='4',font=('arial',12,'bold'),command=lambda:self.get_input(4),bd=5,width=4,pady=10,cursor="hand2").grid(row=2,column=0)
        btn_5 = Button(Cal_Frame, text='5', font=('arial', 12, 'bold'),command=lambda:self.get_input(5), bd=5, width=4, pady=10, cursor="hand2").grid( row=2, column=1)
        btn_6= Button(Cal_Frame, text='6', font=('arial', 12, 'bold'),command=lambda:self.get_input(6), bd=5, width=4, pady=10, cursor="hand2").grid(row=2,column=2)
        btn_sub= Button(Cal_Frame, text='-', font=('arial', 12, 'bold'),command=lambda:self.get_input('-'), bd=5, width=4, pady=10, cursor="hand2").grid(row=2, column=3)


        btn_1=Button(Cal_Frame,text='1',command=lambda:self.get_input(1),font=('arial',12,'bold'),bd=5,width=4,pady=10,cursor="hand2").grid(row=3,column=0)
        btn_2 = Button(Cal_Frame, text='2', command=lambda:self.get_input(2),font=('arial', 12, 'bold'), bd=5, width=4, pady=10, cursor="hand2").grid( row=3, column=1)
        btn_3= Button(Cal_Frame, text='3', command=lambda:self.get_input(3),font=('arial', 12, 'bold'), bd=5, width=4, pady=10, cursor="hand2").grid(row=3, column=2)
        btn_mul= Button(Cal_Frame, text='*',command=lambda:self.get_input('*'), font=('arial', 12, 'bold'), bd=5, width=4, pady=10, cursor="hand2").grid(row=3, column=3)


        btn_0=Button(Cal_Frame,text='0',command=lambda:self.get_input(0),font=('arial',12,'bold'),bd=5,width=4,pady=15,cursor="hand2").grid(row=4,column=0)
        btn_c = Button(Cal_Frame, text='c',command=self.clear_cal,font=('arial', 12, 'bold'), bd=5, width=4, pady=15, cursor="hand2").grid( row=4, column=1)
        btn_eq= Button(Cal_Frame, text='=',command=self.perform_cal,font=('arial', 12, 'bold'), bd=5, width=4, pady=15, cursor="hand2").grid(row=4, column=2)
        btn_div= Button(Cal_Frame, text='/',command=lambda:self.get_input('/'), font=('arial', 12, 'bold'), bd=5, width=4, pady=15, cursor="hand2").grid(row=4, column=3)


        # =======Cart Frame=================

        Cart_Frame = Frame(Cal_Cart_Frame, bd=3,relief=RIDGE, bg = "ivory3")
        Cart_Frame.place(x=280, y=8, width=245, height=315)
        ...
        btn_print=Button(billMenuFrame,text="Print", font=('goudy old style',15,"bold"),bg="gray",fg="white",cursor="hand2")
        ...
        #==footer===
        footer=Label(self.root,text="BILLING SOFTWARE | Devloped By Team NEXA\nfor any Technical issue contact 986544xxyy")
    ...
    def bill_top(self):
        ...
        bill_top_temp=f'''
\t\tBILL-Management 
\t Phone No.7327852324,BBSR-751024
{str("="*47)}
...
{str("="*47)}\n
    ...
    def update_date_time(self):
        ...
        self.lbl_clock.config(text=f"welcome to XYZ Store\t\t Date: {str(date_)}\t\t Time: {str(time_)} ")
